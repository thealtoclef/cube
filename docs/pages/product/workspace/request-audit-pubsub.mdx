...
3. **Pub/Sub Topic** created for audit logs
...
| `query_with_pre_aggregations` | number | Success | Count using pre-aggregations |
...
Create a BigQuery table and use Dataflow to stream audit logs:

**1. Create BigQuery Table**

```sql
CREATE TABLE `project.dataset.cube_audit_logs` (
  event_type STRING,
  event_id STRING,
  ...
  query_type STRING,
  query_count INT64,
  query_with_pre_aggregations INT64,
  cache_type STRING,
  data_source STRING,
  db_type STRING,
  ext_db_type STRING,
...
CLUSTER BY api_type, status;
```

**2. Create Pub/Sub Subscription**

```bash
gcloud pubsub subscriptions create cube-audit-to-bq \
  --topic=audit_requests \
  --ack-deadline=60
```

**3. Start Dataflow Job**

```bash
gcloud dataflow jobs run audit-requests-to-bq \
  --gcs-location=gs://dataflow-templates-us-central1/latest/PubSub_Subscription_to_BigQuery \
  --region=us-central1 \
  --parameters \
inputSubscription=projects/PROJECT_ID/subscriptions/cube-audit-to-bq,\
outputTableSpec=PROJECT_ID:dataset.cube_audit_logs
```

### ClickHouse Consumer

Node.js consumer that writes to ClickHouse:
...
CREATE TABLE IF NOT EXISTS cube_audit_logs (
  event_type String,
  event_id String,
  ...
  query_type String,
  query_count UInt32,
  query_with_pre_aggregations UInt32,
  cache_type String,
  data_source String,
  db_type String,
  ext_db_type String,
  external Boolean,
  last_refresh_time DateTime64(3),
  slow_query Boolean,
  error String
) ENGINE = MergeTree()
ORDER BY (event_created_at, request_id)
PARTITION BY toYYYYMM(event_created_at);
`;

async function startConsumer() {
  const subscription = pubsub.subscription('audit_requests-sub');
  
  subscription.on('message', async (message) => {
    const event = JSON.parse(message.data.toString());
    
    // Transform and insert
    await clickhouse.insert('INSERT INTO cube_audit_logs', [event]).toPromise();
...
FROM `project.dataset.cube_audit_logs`
...
4. **Compliance** - Ensure audit logs meet regulatory requirements (GDPR, HIPAA, etc.)
...
gcloud alpha monitoring policies create \
  ...
  --display-name="Cube Audit Log Errors" \
